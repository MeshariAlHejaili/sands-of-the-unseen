...
public class EnemyObstacleMotor : MonoBehaviour
{
    private const int MaxSweepHits = 32;
    private const int MaxOverlapResults = 32;
    private const int MaxOverlapRecoveryIterations = 2;
    private const float MinMoveDistance = 0.0001f;
    private const float MinCastExtent = 0.001f;

    [Header("Collision Sweep")]
    [Tooltip("Physics layers that stop enemy walking movement.")]
    [SerializeField] private LayerMask blockingLayers = 1 << 9;

    [Tooltip("Small buffer in world units kept between the enemy collider and blockers.")]
    [Range(0.001f, 0.2f)]
    ...
    public Vector3 Move(Vector3 displacement, bool allowSlide)
    {
        ...
        Vector3 moved = MoveSingle(displacement, out RaycastHit blockingHit);
    ...
    private bool TryFindNearestHit(Vector3 direction, float distance, out RaycastHit nearestHit)
    {
        ...
        for (int i = 0; i < hitCount; i++)
        {
            ...
            if (hit.collider == null || IsSelfCollider(hit.collider) || hit.distance >= nearestDistance)
            {
                continue;
            }
